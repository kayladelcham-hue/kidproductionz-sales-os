"""Prepare Batch 2 using GET-only V2 reads; never execute an import."""
import argparse
import copy
...
def refresh(summary_path, cfg, match_cfg, reader, root):
    ...
    for event in events:
        ...
        if status in ('ATTEMPT', 'CREATED', 'CREATE_FAILED_OR_UNKNOWN'):
    ...
    if any(status != 'CREATED' for status in last.values()):
        raise ValueError('Unresolved prior write outcome; reconcile before preparing Batch 2')
    if len(created_ids) < 10:
        raise ValueError('Journal must confirm the ten-company pilot before Batch 2')
    ...
    selected = [r['lead_id'] for r in candidates if r['lead_id'] not in excluded][:50]
    if not selected:
        raise ValueError('No remaining NEW prospects; no plan generated')
    ...
    original = build_plan(summary_path, cfg, schema, selected)
    checks, kept = [], []
    ...
    parent = root / 'v3_imports' / 'batch2_refreshes'
    ...
    report = {**paths,'batch_config':str(final/'batch_config.json'),'snapshot':str(final/'snapshot.json'),'schema':str(final/'schema.json'),
        'report':str(final/'report.json'),'checked':len(selected),'proposed':len(kept),'skipped':len(selected)-len(kept), 'excluded_prior_lead_ids':excluded, 'cap':50,
