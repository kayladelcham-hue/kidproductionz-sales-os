...
class BatchTests(unittest.TestCase):
    ...
    def run_batch(self,statuses=None):
        with patch.object(b,'retrieve',return_value=self.snapshot),patch.object(b,'classify',side_effect=[{'crm_status':s} for s in (statuses or ['NEW']*50)]) as classify:
            result=b.refresh(self.summary,self.f.cfg,self.f.match,self.reader,self.f.root)
            self.assertEqual(classify.call_count,50)
            return result
    def test_excludes_pilot_caps_fifty_metadata(self):
        result=self.run_batch()
        plan=json.loads(b.Path(result['plan']).read_text())
        self.assertEqual([i['lead_id'] for i in plan['items']],[r['lead_id'] for r in self.ordered[10:60]])
    ...
    def test_skips_without_replacement(self):
        result=self.run_batch(['EXISTING','REVIEW']+['NEW']*48)
        self.assertEqual(result['proposed'],48)
        self.assertEqual(result['skipped'],2)
    def test_all_skipped(self):
        self.assertEqual(self.run_batch(['REVIEW']*50)['proposed'],0)
    ...
    def test_incomplete_snapshot_blocks(self):
        ...
        self.assertFalse((self.f.root/'v3_imports'/'batch2_refreshes').exists())
