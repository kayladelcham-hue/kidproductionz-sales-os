...
deals:(campaign?:string,status?:string)=>get<any[]>(`/api/deals?${new URLSearchParams({campaign:campaign||'',status:status||''}).toString()}`),
deal:(id:number)=>get<any>(`/api/deals/${id}`),
updateDeal:(id:number,body:any)=>mutate<any>(`/api/deals/${id}`,'PATCH',body),
newOpportunity:(id:number,body:any)=>post<any>(`/api/customers/${id}/opportunities`,body),
salesRevenue:(campaign?:string,period='30d')=>get<any>(`/api/sales-revenue?period=${encodeURIComponent(period)}${campaign?`&campaign=${encodeURIComponent(campaign)}`:''}`),
defer:(id:number)=>mutate<any>(`/api/prospects/${id}/defer`,'PATCH',{}),
