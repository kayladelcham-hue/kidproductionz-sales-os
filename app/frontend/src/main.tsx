...
const nav=['Overview','Up Next','Lead Generator','Daily Queue','Prospects','Add Prospect','Follow-Up / Schedule Hub','Campaigns','Runs','Settings'];
...
<div className="overview-queue-mobile">
...
</>:<div className="empty small">Loading queueÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦</div>}</div><div><div className="card schedule agenda-card">
  <div className="section-head agenda-head">
    ...
    <button onClick={()=>{
      ...
      api.calendarUpcoming()
        .then(setCalendar)
        .catch(()=>setCalendar({status:"UNAVAILABLE",events:[]}))
  ...
  {calendarLoading
    ...
    : calendar?.status==='UNAVAILABLE'||calendar?.status==='DISABLED'
      ? <p className="muted">Calendar unavailable.</p>

    : calendar?.events?.length
      ? <div className="agenda-list">
          {calendar.events.map((e:any,i:number)=>{
            const d=new Date(e.start||'');
            const prev=i>0?new Date(calendar.events[i-1]?.start||''):null;
            ...
            return <React.Fragment key={e.id||e.html_link||i}>
