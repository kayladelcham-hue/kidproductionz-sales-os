...
import {Dashboard,Appearance,AppNavigation,GuidedTour,Icon,navigationGroups,pageName} from './AppExperience';
...
function App(){const [page,setPage]=useState('Overview');const [loggingOut,setLoggingOut]=useState(false);const handleLogout=async()=>{if(loggingOut)return;setLoggingOut(true);try{await authApi.logout();window.location.reload()}finally{setLoggingOut(false)}};const [tourOpen,setTourOpen]=useState(false);const [hubProspect,setHubProspect]=useState<any>(null);const [mobileMenuOpen,setMobileMenuOpen]=useState(false);const [campaign,setCampaign]=useState('orlando_beauty');const [campaigns,setCampaigns]=useState<any[]>([]);const loadCampaigns=()=>api.campaigns().then((x:any)=>{const rows=Array.isArray(x)?x:(x.campaigns||[]);setCampaigns(rows);if(rows.length&&!rows.some(c=>c.campaign_id===campaign))setCampaign(rows[0].campaign_id)}).catch(()=>setCampaigns([]));useEffect(()=>{loadCampaigns()},[]);useEffect(()=>{document.querySelector('.shell > main')?.scrollTo({top:0});setHubProspect(null)},[page,campaign]);return <div className="shell"><aside inert={mobileMenuOpen||tourOpen}><div className="brand"><img src={logo}/><div><b>KidProductionz</b><small>Sales OS</small></div></div><nav>{navigationGroups.map(g=><section className="xp-desktop-group" key={g.name}><h3>{g.name}</h3>{g.pages.map(n=><button key={n} className={page===n?'active':''} onClick={()=>setPage(n)}>{pageName(n)}</button>)}</section>)}</nav><div className="safe"><span/>System Ready</div><button className="logout-btn" onClick={handleLogout} disabled={loggingOut}>{loggingOut?'Logging out...':'Log out'}</button></aside><main inert={mobileMenuOpen||tourOpen}><header><button className="mobile-menu-btn" aria-label="Open navigation" aria-expanded={mobileMenuOpen} onClick={()=>setMobileMenuOpen(true)}><Icon name="menu"/></button><div><p className="eyebrow">KIDPRODUCTIONZ SALES OS</p><h1>{page==='Settings'?'Settings':pageName(page)}</h1></div><select aria-label="Current campaign" value={campaign} onChange={e=>{setCampaign(e.target.value);setHubProspect(null)}}>{campaigns.map(c=><option key={c.campaign_id} value={c.campaign_id}>{c.name||c.campaign_id}</option>)}</select></header>{page==='Overview'?<Dashboard key={campaign} campaign={campaign} onNavigate={setPage} onTour={()=>setTourOpen(true)}/>:page==='Daily Queue'?<QueuePage campaign={campaign}/>:page==='Up Next'?<UpNext campaign={campaign}/>:page==='Lead Generator'?<LeadGenerator campaign={campaign} onNavigate={setPage}/>:page==='Add Prospect'?<AddProspect key={campaign} campaign={campaign}/>:page==='Follow-Up / Schedule Hub'?<><FollowUpHub key={campaign} campaign={campaign} onOpen={setHubProspect}/>{hubProspect&&<ProspectDrawer item={hubProspect} onClose={()=>setHubProspect(null)}/>}</>:page==='Prospects'?<Prospects campaign={campaign}/>:page==='Campaigns'?<Campaigns campaign={campaign} onChanged={loadCampaigns} onSelect={setCampaign}/>:page==='Runs'?<Runs/>:<Settings campaign={campaign}/>}</main>{mobileMenuOpen&&<AppNavigation page={page} onNavigate={setPage} onClose={()=>setMobileMenuOpen(false)} onTour={()=>setTourOpen(true)}/>}<div className="bottom-nav" inert={mobileMenuOpen||tourOpen}>{[["Home","Overview","home"],["Sell","Up Next","sell"],["Queue","Daily Queue","queue"],["Prospects","Prospects","people"],["Plan","Follow-Up / Schedule Hub","calendar"]].map(([l,v,icon])=><button key={l} aria-current={page===v?'page':undefined} onClick={()=>setPage(v)}><Icon name={icon}/><span>{l}</span></button>)}</div>{tourOpen&&<GuidedTour onNavigate={setPage} onClose={()=>setTourOpen(false)}/>}<SalesAgent campaign={campaign}/></div>}
              ...
              >
                ...
                <div className="prospect-card-main">
                  ...
                  <div className="prospect-card-title">
                    ...
                    {(category||city)&&
                      <small>
                        {[category,city].filter(Boolean).join(' Â· ')}
                      </small>
                    }
                  </div>
                ...
                <div className="prospect-card-footer">
                  ...
                  <span className="prospect-chevron" aria-hidden="true">â€º</span>
                </div>
